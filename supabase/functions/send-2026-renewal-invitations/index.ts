...
function message(firstName: string, actionLink: string, principalNames: string[]) {
...
Deno.serve(async (request) => {
  ...
  const { data: periods, error: periodError } = await supabase.from('enrollment_periods').select('id,coverage_year').eq('program_id', program.id).in('coverage_year', [2025, 2026]);
  ...
  for (const recipient of recipients) {
    ...
    const content = message(firstName, actionLink, recipient.principalNames);
