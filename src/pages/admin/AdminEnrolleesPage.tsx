...
export function AdminEnrolleesPage() {
  ...
  return <>
    ...
    {!loadingPeriod && (rows.length ? <div className="data-table admin-table"><div className="data-table__head"><span>Member</span><span>Plan</span><span>Household</span><span>Total payable</span><span>Hospital</span><span>Status</span></div>{rows.map((enrollment) => <div className="data-table__row" key={enrollment.id}><span data-label="Member"><strong>{fullName(enrollment.principal)}</strong><small>{enrollment.principal.email}</small></span><span data-label="Plan">{periodData.plans.find((plan) => plan.id === enrollment.planId)?.name ?? 'Not selected'}</span><span data-label="Household">{enrollment.dependents.length + 1}</span><strong data-label="Total payable">{formatNaira(enrollment.totalKobo)}</strong><span data-label="Hospital">{enrollment.hospital}</span><span data-label="Status"><StatusBadge status={enrollment.status} /></span></div>)}</div> : <EmptyState icon={<Users size={29} />} title="No matching enrollees" body="Adjust the enrollment year, search, or status filter." />)}
  </>;
}
