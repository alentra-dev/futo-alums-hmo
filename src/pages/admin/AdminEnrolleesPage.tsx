...
export function AdminEnrolleesPage() {
  ...
  return <>
    <PageHeader eyebrow="Administration" title="Enrollees" description="Principal-level enrollment and payment summary." actions={<div className="button-row"><Button variant="secondary" icon={<Download size={18} />} onClick={() => void downloadSummaryWorkbook(snapshot!)}>Summary</Button><Button variant="secondary" icon={<FileSpreadsheet size={18} />} onClick={() => void downloadAdminFullWorkbook(snapshot!)}>Admin full export</Button><Button icon={<FileSpreadsheet size={18} />} onClick={() => void downloadAvonWorkbook(snapshot!)}>AVON export</Button></div>} />
