...
export function AdminDashboardPage() {
  ...
  // Every pending confirmation needs review, whether it is an HMO premium or a program assessment.
  const pending = payments.filter((item) => item.status === 'pending');
  const pendingAssessment = pending.filter((item) => item.assessmentId).length;
  const assessmentVerified = payments.filter((item) => item.status === 'verified' && item.assessmentId).reduce((sum, item) => sum + item.amountKobo, 0);
  ...
  return <>
    ...
    <section className="metric-grid admin-metrics">
      ...
      <article className="metric"><span className="metric__icon"><Banknote size={21} /></span><div><small>Verified HMO collections</small><strong>{formatNaira(verified)}</strong><span>of {formatNaira(expected)} expected{assessmentVerified > 0 ? ` · ${formatNaira(assessmentVerified)} assessment` : ''}</span></div></article>
      <article className="metric"><span className="metric__icon"><CalendarClock size={21} /></span><div><small>Pending review</small><strong>{pending.length}</strong><span>{formatNaira(pending.reduce((sum, item) => sum + item.amountKobo, 0))}{pendingAssessment > 0 ? ` · ${pendingAssessment} assessment` : ''}</span></div></article>
    </section>
    ...
    <section className="table-section"><div className="section-title"><h2>Payments awaiting review</h2><Link to="/admin/payments">Open queue</Link></div>{pending.length ? <div className="review-list">{pending.slice(0, 4).map((payment) => <Link to="/admin/payments" key={payment.id}><span className="person-dot">{payment.principalName[0]}</span><span><strong>{payment.principalName}</strong><small>{payment.assessmentId ? 'Program assessment' : 'HMO premium'} · {payment.reference}</small></span><strong>{formatNaira(payment.amountKobo)}</strong><StatusBadge status={payment.status} /></Link>)}</div> : <EmptyState icon={<CheckCircle2 size={26} />} title="Nothing awaiting review" body="New payment confirmations will appear here as subscribers upload them." />}</section>
  </>;
}
