...
export function AdminPaymentsPage() {
  ...
  return <>
    ...
    {payments.length ? <div className="payment-review-grid">{payments.map((payment) => <article className="payment-review-card" key={payment.id}><div className="payment-review-card__head"><span className="person-dot">{payment.principalName[0]}</span><div><strong>{payment.principalName}</strong><small>{formatDateTime(payment.submittedAt)}</small></div><StatusBadge status={payment.status} /></div><div className="payment-review-card__amount"><small>Amount notified</small><strong>{formatNaira(payment.amountKobo)}</strong></div><dl><div><dt>Date paid</dt><dd>{payment.paidAt}</dd></div><div><dt>Reference</dt><dd>{payment.reference}</dd></div><div><dt>Proof</dt><dd><ReceiptText size={15} />{payment.proofName}</dd></div></dl><div className="payment-review-card__actions"><Button variant="secondary" icon={<Eye size={17} />}>View proof</Button>{payment.status === 'pending' && <><Button variant="danger" icon={<X size={17} />} disabled={busy === payment.id} onClick={() => void review(payment.id, 'rejected')}>Reject</Button><Button icon={<Check size={17} />} disabled={busy === payment.id} onClick={() => void review(payment.id, 'verified')}>Verify</Button></>}</div></article>)}</div> : <EmptyState icon={<ReceiptText size={29} />} title="No payments in this queue" body="Payment notifications matching this filter will appear here." />}
  </>;
}
