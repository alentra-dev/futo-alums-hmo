...
export function PaymentsPage() {
  ...
  return <>
    ...
    <section className="table-section"><div className="section-title"><h2>Payment history</h2><span>{payments.length} notifications</span></div>
      {payments.length ? <div className="data-table"><div className="data-table__head"><span>Date paid</span><span>Amount</span><span>Proof</span><span>Submitted</span><span>Status</span></div>{payments.map((payment) => <div className="data-table__row" key={payment.id}><span data-label="Date paid">{formatDate(payment.paidAt)}</span><strong data-label="Amount">{formatNaira(payment.amountKobo)}</strong><span data-label="Proof"><ReceiptText size={16} />{payment.proofName}</span><span data-label="Submitted">{formatDateTime(payment.submittedAt)}</span><span data-label="Status"><StatusBadge status={payment.status} /></span></div>)}</div> : <EmptyState icon={<ReceiptText size={28} />} title="No payment notifications" body="Payment notifications will appear here after submission." />}
    </section>
