...
import { Banknote, CheckCircle2, ClipboardCheck, Copy, FileUp, Info, ReceiptText } from 'lucide-react';
...
export function PaymentsPage() {
  ...
  return <>
    <PageHeader eyebrow={`${snapshot!.period.year} payments`} title="Payments" description="Notify administrators after every transfer, including partial payments." actions={canNotifyPayment ? <Button icon={<FileUp size={18} />} onClick={() => setOpen(true)}>Notify payment</Button> : <Button icon={<ClipboardCheck size={18} />} onClick={() => location.assign(import.meta.env.BASE_URL + 'enrollment')}>Complete enrollment</Button>} />
    <div className="payment-summary-grid">
      <article className="panel amount-due"><p className="eyebrow">Total payable</p><strong>{formatNaira(enrollment.totalKobo)}</strong><span>Includes the configured AVON NHIS and program administrative fees</span>{premiumKobo > 0 && <FeeBreakdown premiumKobo={premiumKobo} rates={surchargeRates(snapshot!.period)} compact />}<ProgressBar value={enrollment.totalKobo ? (verified / enrollment.totalKobo) * 100 : 0} /><div><span>Verified {formatNaira(verified)}</span><span>Outstanding {formatNaira(outstanding)}</span></div>{pending > 0 && <p className="pending-note"><Info size={16} />{formatNaira(pending)} is awaiting administrator review.</p>}</article>
    ...
    {open && canNotifyPayment && <Modal title="Notify a payment" onClose={() => setOpen(false)}><form className="modal-form" onSubmit={submit}><div className="partial-warning"><Info size={18} /><span>{outstanding === 0 ? 'Your verified payments currently cover the total payable. You can still submit a transaction confirmation that is missing from your payment history.' : 'Full payment is strongly encouraged. Partial payments are accepted and remain due until the verified total covers the full amount.'}</span></div><label>Amount shown on confirmation (₦)<input name="amount" type="number" min="0.01" step="0.01" required placeholder="0.00" /></label><label>Date paid<input name="paidAt" type="date" required defaultValue={new Date().toISOString().slice(0, 10)} /></label><label>Transfer reference<input name="reference" required defaultValue={`${account.referencePrefix} - ${enrollment.principal.firstName} ${enrollment.principal.surname}`} /></label><label className="file-field"><span>Proof of payment</span><input name="proof" type="file" accept="image/jpeg,image/png,application/pdf" required /><small>PDF, JPG, or PNG. Maximum 10 MB.</small></label><div className="modal__actions"><Button type="button" variant="secondary" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" disabled={busy} icon={<CheckCircle2 size={17} />}>{busy ? 'Submitting…' : 'Submit for verification'}</Button></div></form></Modal>}
  </>;
}
