...
import { programDateValue } from '../lib/format';
...
export function PaymentSubmissionForm({ enrollment, account, outstandingKobo, submitLabel = 'Upload confirmation', timezone, onCancel, onSubmit }: PaymentSubmissionFormProps) {
  const today = programDateValue(timezone);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if ((data.get('paidAt') as string) > today) { setError('The date paid cannot be in the future.'); return; }
    setBusy(true);
    setError('');
    try {
      await onSubmit({
  ...
  return <form className="modal-form" onSubmit={submit}>
    ...
    <label>Date paid<input name="paidAt" type="date" required max={today} defaultValue={today} /></label>
